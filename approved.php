...
if(isset($_SESSION['id']) && $_SESSION['id'] == 1){
	...
	include "connect.php";
	$qry = "SELECT `user`.`name`, `jobs`.`jobid`, `jobs`.`title`, `appliedjobs`.`aj_id`  FROM `appliedjobs` JOIN jobs on appliedjobs.jobid = jobs.jobid JOIN user ON appliedjobs.userid = user.userid WHERE `appliedjobs`.`status` = '2'";
	$rsl = $con->query($qry);
?>
<!DOCTYPE html>
<html lang="en">
